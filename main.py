        ...
        if dist:
            ...
            if gamenum != 5:
                ...
                if current_user.is_authenticated:
                    ...
                    db_sess.commit()
                clear_session()
                return render_template('gameresult.html', dist=dist, score=score, pts=f"http://static-maps.yandex.ru/1.x/?ll=99.505405,61.6986538&spn=40,50&pt={coords[1]},{coords[0]}~{x},{y}&l=map", gamescore=gamescore, gamenum=gamenum)
    else:
